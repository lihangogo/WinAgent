...
/**
 *  启动OpenVPN客户端：
 *  仅启用一个OpenVPN客户端，启动过多的客户端反而会造成虚拟网卡错误。
 * 	OpenVPN的所有配置文件均被加密，见util类。
 *  
 *  使用Ping检测客户端是否可用，服务器随主机启动
 *  UDP模拟消息验证VPN是否可用也是可行的
 */
public class Openvpn implements ProxyService {
	...
	//OpenVPN配置文件解密位置
	private static final String deConfigPath = "./conf/openvpn/openvpn";
	//OpenVPN CA文件位置
	...
	class OpenvpnProcessReader implements Callable<Integer> {
		...
		public Integer call() throws InterruptedException {
			...
			try (BufferedReader in = new BufferedReader(new InputStreamReader(process.getInputStream(), Charset.forName("GBK")))) {
